...
- 가격에 영향을 주는 요소
  - 리소스 유형, Consumption(종량제가 아닌 약정), 유지 관리, Geography(지역), 네트워크 트래픽, 구독 유형(12개월 무료), MarketPlace
  - 계산기
    - 요금 계산기 : 프로비저닝된 리소스 기준 계산
    - TCO 계산기 : 온-프레미스로 같은 환경 구축시 필요한 금액 기준 계산
- Azure Cost Management : 청구서를 받기 전에 상황을 인식할 수 있음
  - 비용 분석 : 시각적 기능 및 조직 단위, 월 단위 등의 구분이나 추세등 확인 가능
  - 비용 경고
    - 예산 경고 : 설정한 예산 초과시 알림
    - 크레딧 경고 : 현금 약정 금액이 사용되어 설정 금액 추가시 알림
    - 부서 지출 할당량 경고
  - 태그 : 리소스와 연결된 메타데이터에 따라 체계적으로 유지하고 사용량을 추적
- 거버넌스 및 규정 준수
  - Azure Blueprints(청사진) : 클라우드 구독 또는 환경 배포 표준화. 새 구독시 적용되는 반복가능 설정 등을 정의. 버전 관리가 가능
    - Artifact : 청사진의 구성요소. 역할 할당, 정책할당, Manager 템플릿, 리소스 그룹 등을 설정할 수 있음
  - Azure Policy : 사용자가 정책을 설정하고 위반 시 강조하는 기능.
    - ex) 리소스가 만들어질때 비준수 리소스 만들어 질 경우 자동으로 방지하기
    - Azure Policy 이니셔티브 : 정책 그룹. 관련 정책을 함께 그룹화
  - 리소스 잠금 : 리소스가 삭제되거나 업데이트 되지 않도록 방지
  - Microsoft Service Trust Portal : Microsoft 보안, 개인 정보 보호 및 규정 준수 관행에 관한 다양한 콘텐츠, 도구 등의 정보를 제공하는 포털
- 리소스 관리 및 배포용 기능과 도구 설명
  - Azure Arc : ARM과 함께 작동. Azure, 온-프레미스 및 다중 클라우드 환경을 관리하는 데 도움이 되는 서비스
    - Azure 규정 준수 및 모니터링을 하이브리드 및 다중 클라우드 구성으로 확장할 수 있음
    - 기존 비 Azure 리소스를 Azure Resource Manager에 프로젝팅하여 환경을 관리
    - Azure에서 실행되는 것 처럼 멀티 클라우드 및 하이브리드 VM, 쿠버네티스 Cluster, DB를 관리
  - Azure Resource Manager(ARM) : 배포용 관리 서비스. 리소스를 만들고 업데이트하고 삭제할 수 있는 관리 계층
- Azure 모니터링 도구
  - Azure Advisor : Azure 리소스를 평가하고 여러 권장 사항을 제공
    - 권장사항 범주 : 안정성, 보안, 성능, 운영 우수성, 비용
  - Azure Service Health : 개별 서비스의 상태를 파악하고 추적
    - Azure 상태 : 모든 Azure 지역에 걸친 모든 Azure 서비스 상태의 전역적 보기
    - Service Health : 사용중인 Azure 서비스 및 지역의 상태 범위를 좁혀서 제공
    - Resource Health : 실제 Azure 리소스에 대한 맞춤형 보기
  - Azure Monitor : 소스에 대한 데이터를 수집하고, 해당 데이터를 분석하고, 정보를 시각화하고, 결과에 대한 작업을 수행하는 플랫폼
    - Azure Log Analytics : Azure Monitor에서 수집한 데이터에 대한 로그 쿼리를 작성하고 실행하는 Azure Portal의 도구
    - Application Insights : App을 모니터링. 요청 속도, 사용자, 세션 수 등 다양한 정보 제공
