...
프림은 시작점을 정하고, 시작점에서 가까운 정점을 선택하면서 트리르 구성 하므로 그 과정에서 사이클을 이루지 않지만 
크루스칼은 시작점을 따로 정하지 않고 최소 비용의 간선을 차례로 대입 하면서 트리를 구성하기 때문에 사이클이 이루어지는 항상 확인 해야한다.
...
'''
1922. 네트워크 연결에 진짜 이대로 박아서 V,E만 따로 받게 설정하면 풀림
1647. 도시분할계획은 한번 프림으로 풀어보자.
'''