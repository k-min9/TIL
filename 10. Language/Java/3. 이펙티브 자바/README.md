...
- 전제 : 가장 애매모한 기능(강사님 평가)이라 강의는 다 듣고 단순하게만 정리하고 넘어갔다.
  - cloneable 인터페이스가 마커와 같이 텅 비어있어서 더 애매
- 규약
  - x.clone() != x 는 반드시 true, 즉 클론은 원본과 다른 인스턴스여야 함
  - x.clone().getClass() == x.getClass()는 반드시 true
  - x.clone().equals(x) 는 true가 아닐수도 있다. (id나 이런 부분이 다른 복사일 수 있음)
  - 불변격체라면
    - Cloneable 인터페이스를 구현하고 가장 먼저 super.clone()을 호출한 후 적절하게 수정하며 재정의
- 주의
